...
// Memory store for devices and logs
...
// Background Keep-Alive Interval (Runs every 4 seconds)
setInterval(() => {
    if (isAdminActive()) {
        io.emit('ping');
    }
...
app.get('/api/admin-heartbeat', requireAuth, (req, res) => {
    touchAdminActivity();
    res.json({ status: 'active', activeDevices: devicesMap.size });
});
...
app.post('/admin/refresh-wake', requireAuth, (req, res) => {
    ...
    io.emit('ping');
    notifyDeviceChange();
    addLog(`🔄 ADMIN WAKE/REFRESH: Broadcasted wake ping & refreshed connection state.`);
...
app.get('/admin', requireAuth, (req, res) => {
    ...
    if (devicesMap.size === 0) {
        ...
        devicesMap.forEach((device, id) => {
            ...
            const statusBadge = device.status === 'ONLINE' 
                ...
                : `<span style="background:rgba(255,77,77,0.2); color:#ff4d4d; border:1px solid #ff4d4d; padding:4px 10px; border-radius:12px; font-weight:bold; font-size:12px;">ASLEEP / OFFLINE 💤</span>`;
    ...
    <head>
        ...
        <script>
            ...
            // Automatic admin heartbeat every 4 seconds
            setInterval(function() {
                fetch('/api/admin-heartbeat').catch(function(e){});
            }, 4000);

            // Auto reload page when device status changes or new target connects
            socket.on('device_update', function() {
                window.location.reload();
            });

            // Append live log items to logs terminal
...
io.on('connection', (socket) => {
    ...
    socket.on('disconnect', () => {
        addLog(`🔌 Device Disconnected / Slept: ${deviceObj.model}`);
