...
// Socket.IO server configuration with standard stable ping intervals
...
// Persistent Devices Store (Deduplicated by Device ID/Model/IP)
...
app.post('/admin/refresh-wake', requireAuth, (req, res) => {
    io.emit('ping');
    addLog(`🔄 ADMIN REFRESH: Sent wake ping to all connected target devices.`);
        ...
        <div class="container">
            ...
            <div class="card">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
                    ...
                    <form action="/admin/refresh-wake" method="POST" style="display:inline;">
                        <input type="hidden" name="selected" value="${selectedDeviceId}">
                        <button type="submit" style="background:#00f2fe; color:#000; padding:6px 14px; border-radius:6px; font-weight:bold; font-size:12px; border:none; cursor:pointer;">🔄 REFRESH TABLE</button>
                    </form>
                </div>
...
io.on('connection', (socket) => {
    ...
    // Deduplicate device by ID or Model+IP so it NEVER creates duplicate rows
    ...
    // Update existing device entry (no duplicates!)
