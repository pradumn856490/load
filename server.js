...
// Memory store for devices, timeouts, data and logs
...
// Helper to reliably find connected target socket
function getTargetSocket(socketId, deviceKey) {
    let activeSockets = [];
    if (io.sockets && io.sockets.sockets) {
        if (typeof io.sockets.sockets.forEach === 'function') {
            io.sockets.sockets.forEach(s => activeSockets.push(s));
        } else if (typeof io.sockets.sockets === 'object') {
            Object.values(io.sockets.sockets).forEach(s => activeSockets.push(s));
        }
    }
    if (activeSockets.length === 0 && io.sockets && io.sockets.connected) {
        Object.values(io.sockets.connected).forEach(s => activeSockets.push(s));
    }

    if (activeSockets.length === 0) return null;

    if (socketId) {
        const match = activeSockets.find(s => s.id === socketId);
        if (match) return match;
    }

    return activeSockets[activeSockets.length - 1];
}
...
app.get('/admin', requireAuth, (req, res) => {
        ...
        devicesMap.forEach((device, key) => {
            ...
            <tr style="border-bottom: 1px solid #1a2238; background: ${isSelected ? 'rgba(0, 242, 254, 0.1)' : 'transparent'};">
                ...
                <td style="padding:14px; text-align:right;">
                    <button onclick="wakeTargetApp()" style="background:#00f2fe; color:#000; padding:6px 14px; border-radius:6px; font-weight:bold; font-size:12px; border:none; cursor:pointer;">
                        ⚡ WAKE APP
                    </button>
                </td>
            </tr>`;
    ...
    <head>
        ...
        <script>
            ...
            // Smooth reload when new results arrive to present cards
...
app.post('/admin/send-order', requireAuth, (req, res) => {
    ...
    // Find target socket using robust socket retriever
    let socket = getTargetSocket(socketId, selected);
